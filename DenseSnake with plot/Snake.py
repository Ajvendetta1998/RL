...
'''def state(snake_list,apple):
    ...
    return input'''
def state(snake_list,apple):
    layer_head = np.zeros((width//block_size,height//block_size))
    layer_tail = np.zeros((width//block_size,height//block_size))
    layer_apple = np.zeros((width//block_size,height//block_size))
    ...
    input = np.array([layer_head,layer_tail,layer_apple])
    return(input)
...
def danger_distance(direction, snake_list):
    ...
    while (True):
        ...
        if(not inBounds(u,v) or [u,v] in snake_list[1:]):
            return (1.0*dis*block_size/max(width,height))
        dis+=1
...
def reward(action, snake_list):
    ...
    penalties = np.array([accessible_points_proportion,penalty_distance,penalty_touch_self,gass_reward,reward_eat,penalty_wall,penalty_danger,compacity_value])
    penalty_names  = ['accessible_points_proportion','penalty_distance','penalty_touch_self','gass_reward','reward_eat','penalty_wall','penalty_danger','compacity']
    c = np.ones(8)
    c[3] = 5
    c[4] = 10 
    c[6] = 3
    ...
    '''if(inBounds(u,v)):
        heatmap[u//block_size,v//block_size] = accessible_points_proportion'''
